...
12. **Media queries** and **children**, scoping. `@media`, `& span`, `& > *`. These are _new contexts_ that restart the hierarchy. The comment says "repeat css hierarchy here" because each of these blocks is a fresh ruleset that follows the same order.

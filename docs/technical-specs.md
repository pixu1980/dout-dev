...
* **html-minifier** – Final HTML minification.
...
{% for item in array %}          <!-- Use: <for each="item in array"> -->
{% include 'template' %}         <!-- Use: <include src="template.html"> -->
{% extends 'layout' %}           <!-- Use: <extends layout="layout.html"> -->
{% assign var = value %}         <!-- Use: <assign var="value"> -->
{% endif %}, {% endfor %}, etc.  <!-- Use: </if>, </for>, etc. -->
```
...
4. **HTML Minification**: `html-minifier` safe settings (no attribute removal that could impact a11y/SEO).
...
* **COPILOT\_RULES.md** e **copilot-instructions.md**
...
* **Dates**: Stored as ISO strings; display formatting localized (configurable locale).
...
* Multilingual slug strategy (future i18n milestone) – keep canonical stable?
...
### 19.10 Internazionalizzazione

* **Struttura multi‑lingua (it/en)**

  * Impatto: **Medio** · Sforzo: **Medio** · Dipendenze: nessuna
  * Checklist: cartelle lingua; `hreflang`; switch accessibile; slug per lingua.
...
**Scope**: pagine *Mese*, *Tag* (statiche, generate dal CMS) e *Ricerca* (client‑side).
...
**Stile**: visibile al focus, contrasto AAA, focus ring evidente; invisibile *off-focus* ma non con `display:none`.
...
* Immagine *LCP* può usare `fetchpriority="high"` e non è lazy.
...
* **Navigation API** per intercettare navigazioni interne *non-breaking* dove utile; fallback tradizionale.

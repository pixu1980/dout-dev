import { makePusher } from './pix-highlighter-lex-utils.js';
...
export function lexMarkdown(text) {
  ...
  for (const line of lines) {
    ...
    if (/^(```|~~~)/.test(trimmed)) {
      ...
      else if (inFence && ch === fenceChar) { inFence = false; fenceChar = null; }
      offset += L + 1; // +\n
      continue;
    }

    if (inFence) { push('mdc', offset, offset + L); offset += L + 1; continue; }

    // Heading
    if (/^#{1,6}\s+/.test(line)) { push('mdh', offset, offset + L); offset += L + 1; continue; }

    // Horizontal rule
    if (/^(\s*)([-*_]\s*){3,}$/.test(line)) { push('mdhr', offset, offset + L); offset += L + 1; continue; }

    // Blockquote
    if (/^\s*>\s?/.test(line)) { push('mdbq', offset, offset + L); offset += L + 1; continue; }
    ...
    // Emphasis *...* o _..._
    scanInline(line, /(?:^|[^*])\*([^*\n]+)\*(?!\*)/g, (s, e) => push('mde', offset + s, offset + e));

...
export function lexYAML(text) {
  ...
  while (i < L) {
    ...
    if (/^\s*(---|\.\.\.)\s*$/.test(line)) { push('op', lineStart, lineStart + line.length); i = j + 1; continue; }
    ...
    while (k < line.length) {
      ...
      if (ch === '#') { push('com', lineStart + k, lineStart + line.length); break; }
      k++;
    }
    ...
    while ((m = anchorRe.exec(line))) { push('var', lineStart + m.index, lineStart + m.index + m[0].length); }

    const tagRe = /!![^\s]+/g;
    while ((m = tagRe.exec(line))) { push('type', lineStart + m.index, lineStart + m.index + m[0].length); }
    ...
    while ((m = wordRe.exec(line))) {
      ...
      const e = s + w.length;
      if (/^-?\d/.test(w)) { push('num', s, e); continue; }
      if (bools.test(w)) { push('kw', s, e); continue; }
    }

    const listM = line.match(/^(\s*)-\s+/);
    if (listM) { const p = lineStart + listM[1].length; push('op', p, p + 1); }

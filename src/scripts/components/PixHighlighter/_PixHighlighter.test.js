...
before(async () => {
  ...
  stubClipboard();
  const mod = await import(new URL('./_PixHighlighter.js', import.meta.url));
  ({
    PixHighlighter,
    PIX_HIGHLIGHTER_THEME_OPTIONS,
    enhancePixHighlighters,
...
describe('PixHighlighter', () => {
  ...
  test('keeps enhancing code blocks when customized built-ins are unavailable', async () => {
    ...
    try {
      ...
      await import(new URL(`./_PixHighlighter.js?fallback=${Date.now()}`, import.meta.url));
      await Promise.resolve();
      await Promise.resolve();
  ...
  test('loads component-owned CSS bundles through static registration', async () => {
    const [
      ...
      readFile(new URL('./_PixHighlighter.js', import.meta.url), 'utf8'),
      readFile(new URL('./_PixHighlighter.css', import.meta.url), 'utf8'),

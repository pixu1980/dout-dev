...
before(async () => {
  const componentModule = await import(
    new URL('./_PostFeedLayoutSelector.js?component-test', import.meta.url)
  );
...
describe('PostFeedLayoutSelector', () => {
  ...
  test('loads local component styles and registers them autonomously', async () => {
    ...
    const componentSource = await readFile(
      new URL('./_PostFeedLayoutSelector.js', import.meta.url),
      'utf8'
    );
    const componentCss = await readFile(
      new URL('./_PostFeedLayoutSelector.css', import.meta.url),
      'utf8'
    );
    ...
    assert.ok(
      componentSource.includes("import cssText from 'bundle-text:./_PostFeedLayoutSelector.css';")
    );

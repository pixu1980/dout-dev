...
before(async () => {
  const mod = await import(
    new URL('./_DisplayPreferencesPopover.js?component-test', import.meta.url)
  );
  ({
    BODY_FONT_OPTIONS,
    CODE_FONT_OPTIONS,
...
describe('DisplayPreferencesPopover', () => {
  ...
  test('loads local component styles and registers them autonomously', async () => {
    ...
    const componentSource = await readFile(
      new URL('./_DisplayPreferencesPopover.js', import.meta.url),
      'utf8'
    );
    const componentCss = await readFile(
      new URL('./_DisplayPreferencesPopover.css', import.meta.url),
      'utf8'
    );
    ...
    assert.ok(
      componentSource.includes(
        "import cssText from 'bundle-text:./_DisplayPreferencesPopover.css';"
      )
    );
    assert.ok(componentSource.includes("id: 'open-dyslexic'"));
    assert.ok(componentSource.includes("label: 'OpenDyslexic'"));

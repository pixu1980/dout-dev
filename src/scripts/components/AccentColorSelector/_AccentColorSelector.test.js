...
before(async () => {
  const mod = await import(new URL('./_AccentColorSelector.js?component-test', import.meta.url));
...
describe('AccentColorSelector', () => {
  ...
  test('loads local component styles and registers them autonomously', async () => {
    ...
    const componentSource = await readFile(
      new URL('./_AccentColorSelector.js', import.meta.url),
      'utf8'
    );
    const componentCss = await readFile(
      new URL('./_AccentColorSelector.css', import.meta.url),
      'utf8'
    );
    ...
    assert.ok(
      componentSource.includes("import cssText from 'bundle-text:./_AccentColorSelector.css';")
    );

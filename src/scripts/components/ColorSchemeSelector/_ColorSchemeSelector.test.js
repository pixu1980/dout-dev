...
before(async () => {
  const mod = await import(new URL('./_ColorSchemeSelector.js?component-test', import.meta.url));
...
describe('ColorSchemeSelector', () => {
  ...
  test('loads local component styles and registers them autonomously', async () => {
    ...
    const componentSource = await readFile(
      new URL('./_ColorSchemeSelector.js', import.meta.url),
      'utf8'
    );
    const componentCss = await readFile(
      new URL('./_ColorSchemeSelector.css', import.meta.url),
      'utf8'
    );
    ...
    assert.ok(
      componentSource.includes("import cssText from 'bundle-text:./_ColorSchemeSelector.css';")
    );
